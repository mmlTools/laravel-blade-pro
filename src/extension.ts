...
export function activate(context: vscode.ExtensionContext): void {
  ...
  const engine = new PrettierBladeFormattingEngine(); const selector: vscode.DocumentSelector = [{ language: "laravel-blade", scheme: "file" }, { language: "laravel-blade", scheme: "untitled" }];
  ...
  for (const language of PRETTIER_LANGUAGES) context.subscriptions.push(vscode.languages.registerDocumentFormattingEditProvider([{ language, scheme: "file" }, { language, scheme: "untitled" }], webProvider));
}
