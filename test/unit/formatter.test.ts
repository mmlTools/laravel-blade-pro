...
const options = {
    ...
    printWidth: 120,
    singleQuote: true,
    formatPhpBlocks: true,
    formatEmbeddedCss: true,
    formatEmbeddedJavaScript: true,
    formatEmbeddedTypeScript: true,
...
for (const fixture of [
    ...
    describe(`formatter fixture: ${fixture}`, () => {
        it('matches expected output and is idempotent', async () => {
            ...
            const input = await readFile(join(root, 'input.blade.php'), 'utf8');
            const expected = await readFile(join(root, 'expected.blade.php'), 'utf8');
            const formatted = await engine.format(input, options);
            expect(formatted).toBe(expected);
            expect(await engine.format(formatted, options)).toBe(expected);
        });
    });
}
it('formats a large Blade document responsively', async () => {
    const source = Array.from(
        { length: 500 },
        (_, index) => `@if($item${index})<div>{{$item${index}}}</div>@endif`,
    ).join('\n');
    const started = performance.now();
    await engine.format(source, options);
...
it('formats TypeScript script regions while preserving Blade expressions', async () => {
    const formatted = await engine.format(
        '<script type="text/typescript">const id:number={{$user->id}};</script>\n',
        options,
    );
...
$url = function (array $changes = []) {
$query = array_diff_key($activeQuery, array_flip($remove));
...
`;
    ...
    const formatted = await engine.format(source, options);
    expect(formatted).toBe(expected);
    expect(await engine.format(formatted, options)).toBe(expected);
});
